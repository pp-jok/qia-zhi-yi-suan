...
## 发布通道：v0.3.5 Core Portrait Semantic Presentation
...
Core Runtime 进一步将语义推演与用户展示分离：语义 Bundle 与展示 Bundle 分别指纹化；普通画像会保留高/低方向及情境差异，不泄露内部 Primitive 编号或情境键。它还支持从经资格校验的事实 JSON 构建 Profile，而不在 Skill 内计算命盘；显式解释和审计命令保留完整事实与规则回链。
