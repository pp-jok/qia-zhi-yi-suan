...
## 发布通道：v0.3.7 Qualified Facts Runtime Hardening
...
Core Runtime 进一步将语义推演与用户展示分离：语义 Bundle 与展示 Bundle 分别指纹化；普通画像会保留高/低方向及情境差异，不泄露内部 Primitive 编号或情境键。它只从正式 `deterministic-facts-v1` 资格包构建 Profile，资格、来源和验证结果必须通过，调用方不能自行指定事实 assurance；运行时不计算命盘，显式解释和审计命令保留完整事实与规则回链。
...
发布包不包含二进制文件、固定外部供应商或用户命盘资料。公开的 synthetic 测试只复现候选 IR 与映射边界；Python 模块不计算出生盘。
