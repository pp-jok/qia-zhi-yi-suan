...
from destiny_personality.calculation import (
    AstrologyAspectFact, AstrologyChartFacts, AstrologyPlacement, BaziChartFacts,
    BaziPillar, DeterministicChartFacts, FactMode, HouseCusp, NormalizedBirthTime,
    TimeBasis, TenGodFact, TenGodSourceKind,
)
...
def test_candidate_ir_preserves_context_and_unknown_time_boundaries() -> None:
    ...
    bazi = BaziChartFacts(
        methodology_version="synthetic-v1", year_pillar=BaziPillar("庚", "午"),
        month_pillar=BaziPillar("庚", "午"), day_pillar=BaziPillar("庚", "午"),
        hour_pillar=None, hidden_stems=(),
        ten_gods=(
    ...
    astrology = _astrology(aspects=(AstrologyAspectFact("Uranus", "Sun", "square", 1),), known_time=False)

    profile = build_candidate_core_profile(
        DeterministicChartFacts(_normalized_time(), bazi, astrology), fact_assurance="capability_reported"
    )
    ...
    assert profile.primitive_states["P002"].context_states == {
        "change": "supported_low", "pressure+work": "supported_high",
    }
...
def test_packaged_candidate_assets_match_canonical_assets() -> None:
    ...
    assert {path.name for path in canonical_root.glob("*.yaml")} == {path.name for path in packaged_root.glob("*.yaml")}
    for canonical_path in canonical_root.glob("*.yaml"):
        assert sha256(canonical_path.read_bytes()).digest() == sha256((packaged_root / canonical_path.name).read_bytes()).digest()


def _normalized_time() -> NormalizedBirthTime:
    value = datetime(2000, 1, 1, tzinfo=timezone.utc)
    return NormalizedBirthTime(
        birth_date=date(2000, 1, 1), historical_civil_time=value.replace(tzinfo=None),
        local_standard_time=value.replace(tzinfo=None), utc_time=value,
        true_solar_time=value.replace(tzinfo=None), timezone_name="UTC", dst_was_applied=False,
        time_basis=TimeBasis.STANDARD_TIME, fact_mode=FactMode.STABLE_ONLY,
...
def _astrology(*, aspects, known_time: bool) -> AstrologyChartFacts:
    ...
    placements = tuple(
        AstrologyPlacement(body, Decimal(index * 20), "Aries", Decimal(0), index + 1 if known_time else None)
        for index, body in enumerate(bodies)
    )
    return AstrologyChartFacts(
        methodology_version="synthetic-v1", placements=placements, aspects=aspects,
        ascendant=Decimal(0) if known_time else None, mc=Decimal(90) if known_time else None,
        house_cusps=tuple(HouseCusp(index, Decimal(index * 30)) for index in range(1, 13)) if known_time else (), dignities=(),
    )
